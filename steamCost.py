...
def read_previous_totals():
    files = glob.glob("banana_Price_*.txt")
    if not files:
        return None, None

    latest_file = max(files, key=os.path.getctime)
    previous_total_usd = None
    previous_total_uah = None

    with open(latest_file, "r", encoding="utf-8") as file:
        lines = file.readlines()
        for line in lines:
            if line.startswith("| Total Inventory Value (USD)"):
                parts = line.split("|")
                if len(parts) > 1:
                    try:
                        previous_total_usd = float(parts[1].strip().replace('$', '').strip())
                        previous_total_uah = float(parts[2].strip().replace('UAH', '').strip())
                    except ValueError as ve:
                        print(f"Error parsing previous total values: {ve}")
                        continue

    return previous_total_usd, previous_total_uah


def main():
    steam_id = '76561198302853347'
    app_id = "2923300"
    context_id = "2"

    previous_total_usd, previous_total_uah = read_previous_totals()
    ...
    try:
        ...
        # Сортування за ціною за всі товари в доларах (п'ятий стовпчик)
